...
function latestEntry(entries: OdontogramEntry[], region: string) {
  return entries
    .filter((entry) => entry.toothRegion === region)
    .sort(
      (a, b) =>
...
export function ToothSvg({ toothNumber, entries, onRegionClick }: Props) {
  ...
  const normalizedWholeStatus = wholeEntry?.status?.trim().toLowerCase();
  ...
  const wholePaint =
    wholeEntry && !isExtracted && !isExtractionIndicated
      ? wholeEntry.color
      : undefined;
  ...
  const rootColor = wholePaint
    ? wholePaint
    : latestEntry(toothEntries, "raiz/base")?.color || EMPTY_FILL;
  ...
  return (
    <div className="flex min-w-[52px] flex-col items-center gap-2 select-none">
      {upper && (
